"""
spatial.py — spatial (correlated-background) detector comparison on one scenario.

Detectors
---------
  DART         — global per-pixel denoising-score Rao detector
...
Detection is run for two target signatures per scenario:
  in-patch : the dominant class of the test box (run_inpatch)
  foreign  : a class absent from the box, scaled to the box's mean pixel norm
             (run_foreign). Foreign outputs land in <run>/foreign/.

Per-run deliverables: signatures.pdf, detection_maps.pdf, false-alarm maps,
roc.pdf, per-class Pfa bars, summary_table.csv/.md, metrics.json, scores.npz.

Usage
-----
  python src/spatial.py --config configs/spatial.yaml [--dry-run]
  # or programmatically: src.spatial.run_from_cfg(cfg) / run_multiseed(cfg)
"""
import argparse, copy, json, os, sys, time
from datetime import datetime

_PKG = os.path.dirname(os.path.abspath(__file__))      # .../SDSM/src
...
from src.data import (
    load_hsi, plant_targets, compute_signature, generate_random_boxes,
    scores_to_spatial_map, _crop_pca_box,
    make_whitening as _make_whitening,
    whitened_sigma as _whitened_sigma,
...
from src.models import (
    ScoreNet, dsm_loss, NeighborMLPDenoiser, score_nmlp_additive,
    neighbor_mlp_dsm_loss,
)
...
DEFAULT_CFG = dict(
    ...
    # DARTS-CFAR local normalization
...
def _train_dsm_best(tr_raw, cfg, device):
    ...
    pbar = tqdm(range(E), desc='DART', dynamic_ncols=True, leave=False)
    last = float('nan')
    for ep in pbar:
...
def _train_nmlp_best(tr_raw, tr_nbr, cfg, device):
    ...
    best_loss, best_state = float('inf'), None
    pbar = tqdm(range(E), desc='DARTS', dynamic_ncols=True, leave=False)
    last = float('nan')
    for ep in pbar:
        perm = torch.randperm(P, device=device)
        ep_loss = 0.0; nb = 0
...
def _active_set(cfg):
    ...
    DET_ORDER; otherwise the given subset. 'DARTS-CFAR' implies its base
    'DARTS' is scored too (the CFAR variant is derived from it)."""
    ...
    if 'DARTS-CFAR' in s:
        s.add('DARTS')
    if 'DART-CFAR' in s:        # CFAR variant is derived from the global DSM map
        s.add('DART')
    return s
...
def score_all(pix, nbr, models, tr_raw, tr_nbr, sig, cfg, device, nbr_amf=None):
    ...
    if 'DART' in act and models.get('dsm') is not None:
        out['DART'] = dsm_additive(pix, tr_raw, models['dsm'], sig)
...
def run_detection(sig, sig_label, out_dir, ctx):
    ...
    # ---- DARTS-CFAR: local kNN-Fisher normalization ----
    k_win = int(cfg['k'])
    ...
    if ('DARTS-CFAR' in _active_set(cfg)
            and 'DARTS' in test_scores and models.get('nmlp') is not None):
        test_scores['DARTS-CFAR'] = _knn_fisher_normalize(
            test_scores['DARTS'], models['nmlp'], planted, te_nbr,
            (H_b, W_b), k_win, cfar_lam=lam, use_topk=use_topk,
            win=nmlp_win, guard=nmlp_guard)
        train_scores['DARTS-CFAR'] = _knn_fisher_normalize(
            train_scores['DARTS'], models['nmlp'], tr_raw, tr_nbr,
    ...
    if ('DART-CFAR' in _active_set(cfg) and 'DART' in test_scores):
        dsm_bg    = int(cfg.get('dsm_cfar_window', cfg.get('cfar_bg_window', 11)))
        dsm_guard = int(cfg.get('dsm_cfar_guard', cfg.get('cfar_guard', 3)))
        test_scores['DART-CFAR'] = _cfar_normalize_map(
            test_scores['DART'], (H_b, W_b), dsm_bg, dsm_guard, cfar_lam=lam)
        train_scores['DART-CFAR'] = _cfar_normalize_map(
            train_scores['DART'], tr_shape, dsm_bg, dsm_guard, cfar_lam=lam)
...
def main():
    ...
    data_norm, gt = load_hsi(cfg['dataset'])
