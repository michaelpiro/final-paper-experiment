"""
iid.py — IID experiment pipeline (single-class and multi-class backgrounds).

One call to run_iid(cfg, mode) does everything (mode in {'single', 'multi'}):

  All detectors consume the RAW 103 bands (no PCA, no normalization). The score
  detectors (DART/L-DART and the learned-Rao LRao/L-LRao) carry a FROZEN ZCA
  whitening first layer fit on the training background; they whiten internally
  and return data-space scores, so detection uses the raw signature.

  1. Load the raw cube; build the background / target pixel pools
     (single = one background class; multi = union of all non-target,
     non-excluded classes). Target signature = mean spectrum of the target class.
  2. Shuffle the background once -> train pool of size max(n_train_list) and a
     held-out test pool; plant additive targets once (shared across all n).
  3. Sweep vs n: train L-DART, DART, L-LRao and (optionally) LRao; score the
     test set; also run the classical baselines AMF and GMM-Levin.
  4. Sweep vs rho: at n_fixed_for_rho, retrain the score nets across rho_list.
  5. Save config.yaml, metrics.json, loss_curves.json, scores.npz, figures/*.

Detectors shown: AMF, GMM-Levin, L-DART, DART, L-LRao, LRao.
"""
...
from src.data import (
    load_hsi, compute_sigma_from_data, plant_targets, Whitening,
)
from src.detectors import amf, dsm_additive, gmm_glrt_levin_additive
...
def run_classical_additive(train_raw, test_planted, s_raw, reg_sigma, cfg, mode):
    ...
    Classical baselines used in the paper: AMF and GMM-Levin (the latter shown in
    the multi-class figures, where the background is a genuine mixture)."""
    ...
    all_jobs = {
        ...
        'GMM-Levin': lambda: gmm_glrt_levin_additive(
                         test_planted, train_raw, s_raw, p_steps=50, p_max=1.0),
    }
...
# {AMF, GMM-Levin, DART, L-DART, L-LRao, LRao}, so these six must all differ
# (paper names: DART = MLP score detector, L-DART = linear; LRao = MLP learned
# Rao, L-LRao = linear learned Rao).
DETECTOR_COLORS = {
    'AMF':       '#1f77b4',   # blue
    'GMM-Levin': '#9467bd',   # purple
    'DART':      '#d62728',   # red    — nonlinear (MLP) DART
