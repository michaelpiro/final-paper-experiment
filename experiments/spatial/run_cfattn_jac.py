"""
CF-Attn Jacobian pullback experiment — comparison with base CF-Attn,
NeighborMLP, DSM and classical baselines on the same train/test blocks.

All four models are trained on identical data splits for a fair comparison.

Usage:
    .venv/bin/python -u experiments/spatial/run_cfattn_jac.py
    .venv/bin/python -u experiments/spatial/run_cfattn_jac.py --config experiments/spatial/cfattn_jac.yaml
"""
...
from cfattn_jac_model import (
    CFAttnJacGaussianScoreNet, cfattn_jac_dsm_loss,
    score_cfattn_jac_additive, score_cfattn_jac_replacement,
)
...
# ---------------------------------------------------------------------------
# Helpers (shared with other spatial runners)
# ---------------------------------------------------------------------------
...
def save_bar(metrics, title, path):
    COLORS = ['#9467bd','#d62728','#e377c2','#1f77b4','#aec7e8','#6baed6','#ff7f0e','#08306b']
...
def save_patches(data_norm, gt, cfg, fig_dir):
    H, W, _ = data_norm.shape; bands = (60, 30, 10)
    def _rgb(c):
    ...
    def _plot(arx, agt, box, ttl):
        ...
        arx.set_title(f"{ttl} ({r1-r0}×{c1-c0})")
...
def main():
    ...
    p.add_argument('--config', default=os.path.join(_EXP, 'cfattn_jac.yaml'))
    args = p.parse_args()
    cfg  = yaml.safe_load(open(args.config))

    t0  = time.time()
    ts  = datetime.now().strftime('%Y%m%d_%H%M%S')
    run_dir = os.path.join(cfg['results_dir'], f'cfattn_jac_{ts}')
    ...
    tr_pix, tr_nbr = sub(*crop(cfg['train_box']), cfg['train_n'])
    te_pix, te_nbr = sub(*crop(cfg['test_box']),  cfg['test_n'])
    ...
    # ---- Train helper ----
    def train_model(model, loss_fn, epochs, lr, desc, batch_size=None):
        ...
        hist  = []
        pbar  = tqdm(range(1, epochs+1), desc=desc, dynamic_ncols=True)
        for _ in pbar:
            perm = torch.randperm(P); tot=0.; nb=0
            for i in range(0, P, bs):
                sel = perm[i:i+bs]
                result = loss_fn(model, Xtr[sel], Ntr[sel])
                loss = result[0] if isinstance(result, tuple) else result
                opt.zero_grad(); loss.backward(); opt.step()
                val = result[1] if isinstance(result, tuple) else float(loss)
                tot += val; nb += 1
            sched.step()
            hist.append(tot/max(nb,1))
            pbar.set_postfix(loss=f"{hist[-1]:.3f}",
                             ratio=f"{hist[-1]/baseline:.3f}")
        model.eval()
        return hist

    # ---- 6a. CF-Attn Jac (new) ----
    p_feat = cfg.get('jac_feature_dim', 2 * D)
    print(f"[CF-Attn-Jac] D={D}  p={p_feat}  K={cfg['cfattn_K']}", flush=True)
    jac = CFAttnJacGaussianScoreNet(
        D=D, h=cfg['cfattn_h'], K=cfg['cfattn_K'],
        feature_dim=p_feat, sigma=sigma,
        eps=cfg.get('cfattn_eps', 1e-4))
    km = KMeans(n_clusters=cfg['cfattn_K'], init='k-means++',
                n_init=5, random_state=seed, max_iter=100).fit(tr_pix)
    jac.comp_mu.data.copy_(torch.tensor(km.cluster_centers_, dtype=torch.float32))
    lam_ent = cfg.get('lam_ent', 0.05); lam_div = cfg.get('lam_div', 0.05)
    loss_curves['CF-Attn-Jac'] = train_model(
        jac,
        lambda m,x,n: cfattn_jac_dsm_loss(m,x,n,lam_ent,lam_div),
        cfg['jac_epochs'], cfg['jac_lr'], 'CF-Attn-Jac',
        batch_size=cfg.get('jac_batch', 128))
    torch.save({'state_dict':jac.state_dict(),'cfg':cfg},
               os.path.join(mdl_dir,'cfattn_jac.pt'))

    # ---- 6b. CF-Attn base ----
    print("\n[CF-Attn-base]", flush=True)
    base = CFAttnGaussianScoreNet(
        D=D, h=cfg['cfattn_h'], K=cfg['cfattn_K'],
        sigma=sigma, eps=cfg.get('cfattn_eps',1e-4))
    base.comp_mu.data.copy_(torch.tensor(km.cluster_centers_, dtype=torch.float32))
    loss_curves['CF-Attn-base'] = train_model(
        base,
        lambda m,x,n: cfattn_dsm_loss(m,x,n,lam_ent,lam_div,cfg.get('lam_cov',1e-5)),
        cfg['cfattn_epochs'], cfg['cfattn_lr'], 'CF-Attn-base')
    torch.save({'state_dict':base.state_dict(),'cfg':cfg},
               os.path.join(mdl_dir,'cfattn_base.pt'))

    # ---- 6c. NeighborMLP ----
    print("\n[NeighborMLP]", flush=True)
    ...
    loss_curves['NeighborMLP'] = train_model(
        nmlp, neighbor_mlp_dsm_loss,
        cfg['nmlp_epochs'], cfg['nmlp_lr'], 'NeighborMLP',
        batch_size=cfg['nmlp_batch'])
    ...
    opt_d = torch.optim.Adam(dsm_net.parameters(), lr=cfg['dsm_lr'],
                              weight_decay=cfg['weight_decay'])
    for _ in pbar:
    ...
    for tm in ('additive', 'replacement'):
        ...
        if tm == 'additive':
            sc_jac  = score_cfattn_jac_additive(jac,  planted, te_nbr_f, tr_pix, tr_nbr_f, s_pca)
            sc_base = score_cfattn_additive(    base,  planted, te_nbr_f, tr_pix, tr_nbr_f, s_pca)
            ...
            sc_jac  = score_cfattn_jac_replacement(jac,  planted, te_nbr_f, tr_pix, tr_nbr_f, s_pca)
            sc_base = score_cfattn_replacement(    base,  planted, te_nbr_f, tr_pix, tr_nbr_f, s_pca)
    ...
    save_bar(metrics, f"CF-Attn-Jac (p={p_feat}) vs baselines — {dom_name} target",
             os.path.join(fig_dir,'auc_bar.pdf'))
    save_loss(loss_curves, os.path.join(fig_dir,'loss_curves.png'))
